...
namespace Univer\Blog\Model\ResourceModel;

use Univer\Blog\Model\Post as PostModel;
...
class Post extends AbstractResourceModel
{
    ...
    {
        $this->_init('univer_blog_post', 'post_id');
    }
    ...
    {
        return $this->getStoreIds('univer_blog_post_store', 'post_id', $id);
    }
    ...
    protected function lookupCategoryIds($id)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('univer_blog_post_category'),
            'category_id'
    ...
    protected function lookupTagIds($id)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('univer_blog_tag_post'),
            'tag_id'
        )->where('post_id = ?', (int)$id);
        return $connection->fetchCol($select);
    }
    ...
    protected function lookupProductIds($id)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('univer_blog_post_product'),
            'product_id'
        )->where('post_id = ?', (int)$id);
        return $connection->fetchCol($select);
    }
    ...
    {
        $table = $this->getTable('univer_blog_post_store');
    ...
    {
        $table = $this->getTable('univer_blog_post_category');
    ...
    {
        $table = $this->getTable('univer_blog_tag_post');
    ...
    {
        $table = $this->getTable('univer_blog_post_product');
    ...
    public function checkUrlKeyAndGetId($url_key)
    {
        ...
        return $this->getObjectIdByStore($postIds, 'univer_blog_post_store', 'post_id');
    }
    ...
    protected function _beforeSave(AbstractModel $object)
    {
        ...
        if (!$this->checkUniqueUrlKey($object, $existingIds, 'univer_blog_post_store', 'post_id')) {
    ...
    public function lookupAuthorFullName($authorId)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('univer_blog_author'),
    ...
    public function getAuthorUrlKey($authorId)
    {
        ...
        $postUrl = (!empty($prefixUrl)) ? '/' . trim($prefixUrl) . '/author/' . $urlKey : '/univer_blog' . '/author/' . $urlKey;
        return $postUrl;
    }
}
